...
# Everything a record needs is on the `session_meta` line that opens a file,
# so a chunk that starts after it cannot be read on its own.
...
def read(transcript: Transcript) -> Iterator[dict[str, Any]]:
    ...
    for offset, entry in enumerate(entries(transcript.lines)):
        index = transcript.entries_before + offset
        ...
        yield from _entry(entry, payload, session, rollout, index, transcript.file_id)
